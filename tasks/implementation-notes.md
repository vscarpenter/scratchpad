...
- The shell now takes its semantic surface colors and flat geometry from the base cascade: zero shell gap/padding, no sidebar/main radius, shadow, or backdrop filter, and a single desktop divider. Legacy mobile rounding and duplicate desktop surface overrides were deleted; dialogs and the document card retain their intentional elevation.
- Computed-style contracts passed at 390, 768, and 1440 pixels across Chromium, Firefox, and WebKit. Nine deterministic screenshots also verified light/dark desktop, tablet, and mobile list/editor states without clipping or surface regressions.
- The UI detector's existing gradient-text warning was recorded as a file-scoped sanctioned exception for the static About-page accent treatment. That treatment and the intentionally stale `.impeccable/design.json` remain outside this refactor's approved scope.
