...
test.describe('typography — serif headings and matched read/edit metrics', () => {
  test('title and rendered headings use the serif stack, code stays mono', async ({ page }) => {
    await seedRawNotes(page, [
      { id: 'serif-note', title: 'Serif title', body: '## Section\n\nBody paragraph.\n\nUses a `code span` too.' },
    ]);

    const title = page.locator('#note-title-display');
    expect(await title.evaluate((el) => getComputedStyle(el).fontFamily)).toContain('Iowan Old Style');
    expect(await title.evaluate((el) => getComputedStyle(el).fontWeight)).toBe('400');

    const h2 = page.locator('#note-rendered h2');
    expect(await h2.evaluate((el) => getComputedStyle(el).fontFamily)).toContain('Iowan Old Style');
    expect(await h2.evaluate((el) => getComputedStyle(el).fontSize)).toBe('24px');
    expect(await h2.evaluate((el) => getComputedStyle(el).fontWeight)).toBe('400');
